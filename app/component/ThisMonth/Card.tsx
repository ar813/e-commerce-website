...
const Card = ({ heading, newPrice, oldPice, rankingNumber, color, picture, width }: Card) => {
    ...
    return (
        <div className="grid gap-[16px]" onMouseEnter={() => { setValue(true) }} onMouseLeave={() => { setValue(false) }}>
            ...
            <Link href={"/productDetailedPage"}>
                    <Image src={`/images/thisMonth/${picture}.svg`} alt={picture} width={100} height={100} className={`${width} m-auto h-[250px]`}></Image>
            </Link>
                </div>
